...
class FormCore {
	...
	protected function step3() {
		...
		// 何らかの処理
		$data = $this->step3_other_process($data);
		...
		if($this->mode==1) {
			foreach ($this->fieldSet as $key=>$value) {
				if(isset($value['list'])===true) {
					$set = array_search($value['default'], $value['list']) === false ? '' : array_search($value['default'], $value['list']) + 1;
					for($i=1;$i<=count($value['list']);$i++) {
						if($set !== '' && $i == $set) {
							$default[$key.'_'.$i] = true;
						}
						else {
							$default[$key.'_'.$i] = false;
						}
					}
				}
				else {
	 ...
	 * @return mixed
	 */
	public function step3_other_process($data) {
		return $data;
	}
...
//exit();
		...
		// メール送信
		$send_error = false;
		if ($this->sendMail($to, $mailConfig['mail']['from'], $mailConfig['mail']['from_name'], $mailConfig['mail']['subject'], $text) == false) {
			if(USE_LOG) $this->log($to);
			$send_error = true;
		}
		...
		if ($mailConfig['mail']['to_manager']) {
			for($i=0;$i<count($to_manager);$i++) {
				if ($this->sendMail($to_manager[$i], $to, '', $mailConfig['mail']['to_manager_subject'], $text2) == false) {
					if(USE_LOG) $this->log($to_manager[$i]);
					$send_error = true;
				}
			}
		}
	...
	public function log($email) {
		$error_text = '送信失敗-->'.date('Y/m/d H:i:s').', ['.$email.']'."\n";
		file_put_contents(LOG_DIR.LOG_NAME, $error_text, FILE_APPEND | LOCK_EX);
	}
