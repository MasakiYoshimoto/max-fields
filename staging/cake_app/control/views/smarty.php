 ...
 *
 * update: 10/27/06 by tclineks
 *  - added plugins_dir ( VIEWS.'smarty_plugins'.DS ), register_functions and _getViewFileName override
 *  - @see http://bakery.cakephp.org/articles/view/124 for usage details
 */
 ...
 */
	function _getViewFileName($action) {
		...
		if ($viewFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . 'errors' . DS . $type . $action . '.ctp')) {
			...
			$viewFileName = $this->Smarty->template_dir . $this->viewPath . DS . $this->subDir . $type . $action . $this->ext;
