...
// プラグインの登録
	$this->registerPlugins('before','smartyanytime_plugins_job2','start');

// Smartyプラグインの登録
	$this->smarty->register_function('job2_list', array('smartyanytime_plugins_job2','job2_list'), false);
	$this->smarty->register_function('job2_detail', array('smartyanytime_plugins_job2','job2_detail'), false);
	$this->smarty->register_function('new_job2_list', array('smartyanytime_plugins_job2','new_job2_list'), false);

	class smartyanytime_plugins_job2 extends smartyanytime_plugins {
		...
		function job2_list($params, &$smarty) {
			define('USE_MYSQLI', false);
			$wwwroot = $smarty->smartyanytime['info']['wwwroot'];
			$wwwroot = ($wwwroot=='/') ? (''):($wwwroot);

			$cms = $smarty->smartyanytime['plugins']['smartyanytime_plugins_job2'];
			...
			// categoryチェック
			$category = array();
			require(realpath(SMARTYANYTIME_DIR.'/../cake_app/category3.php'));
			...
			// areaチェック
			$area = array();
			require(realpath(SMARTYANYTIME_DIR.'/../cake_app/area2.php'));
		...
		function new_job2_list($params, &$smarty) {
			define('USE_MYSQLI', false);
			$wwwroot = $smarty->smartyanytime['info']['wwwroot'];
			$wwwroot = ($wwwroot=='/') ? (''):($wwwroot);
		...
		function job2_detail($params, &$smarty) {
			define('USE_MYSQLI', false);
			$dummy=array();
			$cms = $smarty->smartyanytime['plugins']['smartyanytime_plugins_job2'];
...
//			}
		...
		function getJobNam ($job, $area, $job_null) {
			...
			$sql = 'SELECT * FROM '.DB_PREFIX.'job2_offers WHERE disable = 0 AND deleted = 0 '.$job_sql;
			$sql .= $area_sql;
			$sql .= ' AND ';
		...
		function getNewJobData ($limit) {
			...
			$sql = 'SELECT * FROM '.DB_PREFIX.'job2_offers WHERE disable = 0 AND deleted = 0 ';
			$sql .= ' AND ';
			$sql .= '(open_date <= "'.$now_date.'" OR !DAYOFMONTH(open_date)) AND ';
			$sql .= '(end_date >= "'.$now_date.'" OR !DAYOFMONTH(end_date)) ';
			$sql .= $sort_sql;
		...
		function getJobDetailData ($id) {
			...
			$sql = 'SELECT * FROM '.DB_PREFIX.'job2_offers WHERE id="'.$id.'" AND ';
			$sql .= '(open_date <= "'.$now_date.'" OR !DAYOFMONTH(open_date)) AND ';
