...
class obAuthComponent extends Object 
{
	...
	var $user_fields = array('id' => 'id', 'username' => 'username', 'password' => 'password', 'group_id' => 'group_id', 'fault_cnt' => 'fault_cnt');
	...
	function login($data) 
	{
		...
		if (empty($user)) {
			...
			$sessdata["{$this->user_model}"]['fault_cnt'] = $user["{$this->user_model}"]["{$this->user_fields['fault_cnt']}"];
...
//		$this->controller->redirect($page);
		$this->controller->render($page); // レンダーさせる
	}
